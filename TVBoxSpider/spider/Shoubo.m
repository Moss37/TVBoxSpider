...
@implementation Shoubo

- (instancetype)init
{
    self = [super init];
    if (self) {
        _siteUrl = @"https://www.shoubokp.com";
        _siteHost = @"shoubokp.com";
    }
    return self;
}
...
- (NSRegularExpression *)regexCategory {
    ...
    return [NSRegularExpression regularExpressionWithPattern:@"/video/(\\S+).html" options:NSRegularExpressionCaseInsensitive error:nil];
}

- (NSRegularExpression *)regexVid {
    return [NSRegularExpression regularExpressionWithPattern:@"/del/(\\d+).html" options:NSRegularExpressionCaseInsensitive error:nil];
}
...
- (NSRegularExpression *)regexDetailVid {
    return [NSRegularExpression regularExpressionWithPattern:@"/vplay/(\\d+)/(\\d+)/(\\d+).html" options:NSRegularExpressionCaseInsensitive error:nil];
}

- (NSRegularExpression *)regexPage {
    return [NSRegularExpression regularExpressionWithPattern:@"/show/(\\d+)--------(\\d+)---.html" options:NSRegularExpressionCaseInsensitive error:nil];
}
...
- (void)homeContent:(BOOL)filter completion:(void(^)(NSString *))completion {
    @try {
        ...
        [self fetchDocumentWithURL:url completion:^(GDataXMLDocument *doc) {
            ...
            // 处理视频列表
            NSArray *vodNodes = [doc nodesForXPath:@"//ul[@class='myui-vodlist clearfix']/li[contains(@class,'col-lg-6 col-md-6 col-sm-4 col-xs-3')]/div[@class='myui-vodlist__box']" error:nil];
            NSMutableArray *videos = [NSMutableArray arrayWithCapacity:vodNodes.count];
            
            NSArray *tuijianNodes = [doc nodesForXPath:@"//div[@class='col-lg-6 col-md-6 col-sm-4 col-xs-3']/div[@class='myui-vodlist__box']" error:nil];
            for (GDataXMLElement *vodNode in tuijianNodes) {
                GDataXMLElement *thumbNode = [[vodNode nodesForXPath:@".//a[@class='myui-vodlist__thumb']" error:nil] firstObject];
                ...
                if (vidMatch) {
                    ...
                    GDataXMLNode *titleNode = [[vodNode nodesForXPath:@".//div[@class='myui-vodlist__detail']/h4[@class='title text-overflow']/a/text()" error:nil] firstObject];
//                    background: url(https://img.lzzyimg.com/upload/vod/20240410-1/96ecf491e81455bf9c402ced652b3ab7.jpg);
                    GDataXMLElement *imgNode = [[thumbNode nodesForXPath:@".//@style" error:nil] firstObject];
                    NSString *styleText = imgNode.stringValue;
                    NSString *vod_pic = @"";
                    NSTextCheckingResult *vidMatch = [self.regexUrl firstMatchInString:styleText
                                                                               options:0
                                                                                 range:NSMakeRange(0, styleText.length)];
                    if (vidMatch) {
                        vod_pic = [styleText substringWithRange:[vidMatch rangeAtIndex:1]];
                    }
                    if ([vod_pic hasPrefix:@"("]) {
                        vod_pic = [vod_pic stringByReplacingOccurrencesOfString:@"(" withString:@""];
                    }
                    if ([vod_pic hasSuffix:@")"]) {
                        vod_pic = [vod_pic stringByReplacingOccurrencesOfString:@")" withString:@""];
                    }
...
- (void)categoryContent:(NSString *)tid page:(NSString *)pg filter:(BOOL)filter extend:(NSDictionary<NSString *, NSString *> *)extend completion:(void(^)(NSString *))completion {
    @try {
        __block NSString *url = [self.siteUrl stringByAppendingString:@"/show/"];
...
- (void)detailContent:(NSArray<NSString *> *)ids completion:(void(^)(NSString *))completion {
    @try {
        // 构建视频详情URL
        NSString *url = [NSString stringWithFormat:@"%@/del/%@.html", self.siteUrl, ids.firstObject];
        [self fetchDocumentWithURL:url completion:^(GDataXMLDocument *doc) {
            NSMutableDictionary *result = [NSMutableDictionary dictionary];
            ...
            NSString *cover = [[[doc nodesForXPath:@"//div[contains(@class,'myui-content__thumb')]/a[@class='myui-vodlist__thumb img-md-220 img-sm-220 img-xs-130 picture']/img" error:nil] firstObject] attributeForName:@"data-original"].stringValue;
            ...
            NSString *desc = [[doc firstNodeForXPath:@"//meta[@name='description']/@content" error:nil].stringValue stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
...
//        headers[@"origin"] = @"https://jumi.tv";
        ...
        // 构建播放页URL
        NSString *url = [NSString stringWithFormat:@"%@/vplay/%@.html", self.siteUrl, videoId];
        [self fetchDocumentWithURL:url completion:^(GDataXMLDocument *doc) {
